...
class SLMAdversarialLoss(torch.nn.Module):
    ...
    def forward(
        ...
        d, _ = self.model.predictor(
            ...
            torch.randn(ref_lengths.shape[0], ref_lengths.max(), 2).to(ref_text.device), text_mask)
        ...
        for _s2s_pred, _text_length in zip(d, ref_lengths):
            ...
            out = torch.nn.functional.conv1d(
                _s2s_pred_org.unsqueeze(0), h.unsqueeze(1), padding=h.shape[-1] - 1,
                groups=int(_text_length))[..., :l]
        ...
        if (iters + 1) % self.skip_update == 0:
            ...
            if use_rec:  # use reconstructed (shorter lengths), do length invariant regularization
                if wav.size(-1) > y_pred.size(-1):
                    ...
                    if np.random.randint(0, 2) == 0:
                        d_loss = self.wl.discriminator(real_GP.detach().squeeze(),
                                                       y_pred.detach().squeeze()).mean()
                    else:
                        d_loss = self.wl.discriminator(wav.detach().squeeze(),
                                                       y_pred.detach().squeeze()).mean()
                else:

...
def analyze_directory(directory, delete_long_files=False):
    ...
    if delete_long_files and deleted_files:
        durations = [
            calculate_audio_duration(os.path.join(directory, f)) for f in os.listdir(directory)
            if f.endswith('.wav')
        ]
    ...
    if durations:
        ...
        stats['exceeding_percentage_7s'] = (
            len(exceeding_files_7s) / len(durations) * 100) if durations else 0
        ...
        stats['exceeding_percentage_30s'] = (
            len(exceeding_files_30s) / len(durations) * 100) if durations else 0
    else:
        stats = {
            'count': 0,
            'min': 0,
...
def main():
    ...
    for directory in directories:
        ...
        if durations:
            ...
            print(
                f"Files exceeding 7 seconds: {stats['exceeding_count_7s']} ({stats['exceeding_percentage_7s']:.2f}%)"
            )
            print(
                f"Files exceeding 30 seconds: {stats['exceeding_count_30s']} ({stats['exceeding_percentage_30s']:.2f}%)"
            )
    ...
    if all_stats:
        ...
        print(
            f"Total files exceeding 7 seconds: {total_exceeding_7s} ({total_exceeding_7s/total_files*100:.2f}% if any)"
        )
        print(
            f"Total files exceeding 30 seconds: {total_exceeding_30s} ({total_exceeding_30s/total_files*100:.2f}% if any)"
        )

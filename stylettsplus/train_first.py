...
def main(config_path):
    ...
    optimizer = build_optimizer({key: model[key].parameters()
                                 for key in model},
                                scheduler_params_dict={key: scheduler_params.copy()
                                                       for key in model},
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Reduce batch size by half after epoch 50
        if epoch == 50 and accelerator.is_main_process:
            logger.info(
                f"Reducing batch size from {batch_size} to {batch_size//2} at epoch 50 to prevent GPU memory errors"
            )
            # Update batch size for subsequent epochs
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                attn_mask = attn_mask.float() * (~text_mask).unsqueeze(-1).expand(
                    text_mask.shape[0], text_mask.shape[1], mask.shape[-1]).float()
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                with torch.no_grad():
                    ...
                    attn_mask = (~mask).unsqueeze(-1).expand(
                        mask.shape[0], mask.shape[1], text_mask.shape[-1]).float().transpose(-1, -2)
                    attn_mask = attn_mask.float() * (~text_mask).unsqueeze(-1).expand(
                        text_mask.shape[0], text_mask.shape[1], mask.shape[-1]).float()
                    attn_mask = (attn_mask < 1)
                    s2s_attn.masked_fill_(attn_mask, 0.0)

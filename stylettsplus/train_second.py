...
def main(config_path):
    ...
    if not load_pretrained:
        if config.get('first_stage_path', '') != '':
            ...
            model, _, start_epoch, iters = load_checkpoint(
                ...
                ignore_modules=[
                    'bert', 'bert_encoder', 'predictor', 'predictor_encoder', 'msd', 'mpd', 'wd', 'diffusion'
    ...
    optimizer = build_optimizer({key: model[key].parameters()
                                 for key in model},
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if epoch >= diff_epoch:
                ...
                if multispeaker:
                    ...
                    loss_diff = model.diffusion(
                        s_trg.unsqueeze(1), embedding=bert_dur, features=ref).mean()  # EDM loss
            ...
            if epoch >= joint_epoch:
                ...
                slm_out = slmadv(
                    i, y_rec_gt, y_rec_gt_pred, waves, mel_input_length, ref_texts, ref_lengths, use_ind,
                    s_trg.detach(), ref if multispeaker else None)
            ...
            if (i + 1) % log_interval == 0:
                logger.info(
                    ...
                    % (
                        ...
                        d_loss, loss_dur, loss_ce, loss_norm_rec, loss_F0_rec, loss_lm, loss_gen_all,
        ...
        logger.info(
            'Validation loss: {:.3f}, Dur loss: {:.3f}, F0 loss: {:.3f}'.format(
            ...
            with torch.no_grad():
                ...
                for bib in range(len(d_en)):
                    if multispeaker:
                        s_pred = sampler(
                            ...
                            features=ref_s[bib].unsqueeze(
                                0),  # reference from the same speaker as the embedding
                    ...
                    d = model.predictor.text_encoder(
                        d_en[bib, :, :input_lengths[bib]].unsqueeze(0), s, input_lengths[bib,
                                                                                         ...].unsqueeze(0),
